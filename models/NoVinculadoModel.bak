...
class NoVinculadoModel extends ModelBase {
    ...
    private function getPerfilNoVinculado(){
        $consult = $this->db->executeQue("select * from perfiles where grupo='No vinculado'");
    ...
    private function correoNoVinculado($correo, $usuario, $alias, $cedula, $pass, $phone, $direccion, $barrio, $ciudad, $fechacumple) {
        ...
        $email2->subject("Nuevo usuario No vinculado");        
        ...
        $bodymail2.='<p>Se ha registrado un nuevo usuario como No vinculado, a continuacion se encuentra la informacion basica y de contacto del nuevo usuario:</p>';
