...
class AssociatedModel extends ModelBase {
    ...
    public function frmPatrocinador($user, $namesponsor) {
        ...
        $bodymail.= '<tr><td><p>
        Nos permitimos informarle que el señor(a)<strong>' . $user->getNombre() . '</strong> se ha inscrito en su organizacion y ahora forma parte de nuestra gran familia REDSOL SAS.
        ha sido inscrito en su red de negocios, por lo cual extendemos nuestras felicitaciones; asi mismo le animamos a que inmediatamente inicie con su usuario el proceso de capacitacion. 
        A continuacion se lista la informacion de su nuevo usuario:    
        </p></td></tr>';
    ...
    public function frmVinculado($user, $codigo) {
        ...
        $bodymail.= '<tr><td>Código de Estudiante independiente:   <strong>' . $user->getId() . '</strong></td><td align="left"></td></tr>';
...
*/
    public function createAndSavePdf($info) {
        ...
        $pdf->Cell(40, 22, "084 827 851", 'LTRB', 0, 'C', false);
        $pdf->Cell(40, 22, "Banco AV VILLAS", 'LTRB', 0, 'C', false);
        $pdf->Cell(40, 22, "FundRed", 'LTRB', 0, 'C', false);
        $pdf->Ln();        
