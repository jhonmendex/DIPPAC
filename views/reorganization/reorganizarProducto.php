...
<div class="container">
    <fieldset class="colorleyend" style="width: 95%;">
        <legend class="colorleyendinto">Paso 1: Selección del producto a reorganizar</legend>
        <div id="inputreferencia">
            <div style="float: left;line-height: 20px">
                Ingrese la referencia  del producto a reorganizar:
            </div>
                        ...
                        if (sizeof($items) != 0) {
                            foreach ($items as $value) {
                                ...
                                <tr class="class<?php echo $estilo; ?>" id="<?php echo $value['id']; ?>">
                                    ...
                                    <td align="center" class="item2" id="stoimportante<?php echo $value["id"] ?>">
                                    ...
                                    <td align="center" class="item2" id="nameb<?php echo $value["id"] ?>">
                                        ...
                                        if ($value['unidad'] == 'und') {
                                            $view->input("cantidad", "numeric", "cantidad", array('required' => true, 'text' => 'numeric', 'minsize' => '1'), array('maxlength' => '3',
                                                ...
                                                "item" => $value["id"],
                                                "class" => "evento",
                                                "value" => isset($_SESSION['cantsesion']) ? $_SESSION['cantsesion'] : null));
                                        } else {
                                    ...
                                    <td align="center" class="item2" id="nameb<?php echo $value["id"] ?>">
                                        <a id="dell<?php echo sha1($value['id']); ?>"
                                           ...
                                           tar="index.php?controlador=Reorganization&accion=deleteItemp"
    ...
    <fieldset class="colorleyend" style="width: 95%; margin-top: 30px">
        <legend class="colorleyendinto">Paso 2: Selecci&oacute;n de productos</legend>
        <div id="cajaselect" style="margin-bottom: 15px">
            <table border="0" width="100%">
                <tr>
                    <td><div style="width: 100%; font-size: 15px; line-height: 17px ; color: #993300;  float: left">Seleccione los articulos en los que quiere clasificar del producto: <h3 style="margin: 1px" id="nombrep2"></h3></div></td>
                    ...
                    foreach ($detalles[0] as $value) {
                        ...
                        <tr class="class<?php echo $estilo; ?>" id="none<?php echo $value["id"] ?>">
                            ...
                            <td class="item2" style="width: 40px;;text-align: center" id="price<?php echo $value["id"] ?>">
                                <?php
                                if ($value['unidad'] == 'und') {
                                    if (isset($_SESSION['Reorgsesion'][$value["id"]])) {
                                        $view->input("cantdev", "numeric", "cantidad devolucion", array('required' => true, 'text' => 'numeric', 'minsize' => '1'), array('size' => '5',
                                            ...
                                            "value" => $_SESSION['Reorgsesion'][$value["id"]]["cantid"]));
                                    } else {
                                        $view->input("cantdev", "numeric", "cantidad devolucion", array('required' => true, 'text' => 'numeric', 'minsize' => '1'), array('size' => '5',
                                            'maxlength' => '5',
                                            "onkeyup" => "actualizar($(this).attr(\"id\"),$(this).attr(\"item\"))",
                            ...
                            <td class="item2" style="width: 20px;text-align: center">
                                <?php if (isset($_SESSION['Reorgsesion'][$value["id"]])) {
                                    if ($_SESSION['Reorgsesion'][$value["id"]]["estado"] == 'Remover') { ?>
                                        <button style="background-color: #ff4040 " item ="<?php echo $value["id"] ?>" class="buscarButton" id="agregarpr<?php echo $value["id"] ?>" idck="<?php echo $value["id"] ?>"  onclick="seleccionar($(this).attr('id'),$(this).attr('item'))" > <?php echo $_SESSION['Reorgsesion'][$value["id"]]["estado"]; ?></button>
                                    <?php } else { ?>
                                        <button item ="<?php echo $value["id"] ?>" class="buscarButtonDis" id="agregarpr<?php echo $value["id"] ?>" idck="<?php echo $value["id"] ?>" disabled="disabled"  onclick="seleccionar($(this).attr('id'),$(this).attr('item'))" > <?php echo $_SESSION['Reorgsesion'][$value["id"]]["estado"]; ?></button>
                                    <?php } ?>
                                <?php } else { ?>
    ...
    <div style="float: left;height: 40px; margin-left: 5px">
        <a id="allcancel"
           tar="index.php?controlador=Reorganization&accion=cancelarsesion"
           href="#"
...
<script>
    function guardarcant(){
        $.ajax({
            type: "POST",
            url: 'index.php?controlador=Reorganization&accion=guardarCant',
            dataType: "json",
            data: {cajas:($("#areorgn input[name='cantidad']").val())}
        });
    }

    $('#nombrep2').html($('#nombrep').html())
    $('#nombrep3').html($('#nombrep').html())
    $('#nombrep4').html($('#nombrep').html())
    ...
    function siguiente(){
        ...
        if($(".dataTables_empty").val()==''){
            parent.message("Debe agregar el producto a reorganizar para poder continuar","images/iconos_alerta/error.png");
            ...
            if(validates("areorgn","no")){
                if(parseFloat($.trim($("#areorgn input[name='cantidad']").val()))>parseFloat($.trim($("#stoimportante"+$.trim($("#areorgn input[name='cantidad']").attr("item"))).html()))){
                    parent.message("La cantidad del producto a reorganizar es mayor a las existencias en stock","images/iconos_alerta/error.png");
                    $('#loader').css('display', 'none');
                    $("#siguiente").removeAttr("disabled");
                    $("#siguiente").addClass("buscarButton");
                    $("#siguiente").removeClass("buscarButtonDis");
                    ...
                    $.ajax({
                        type: "POST",
                        url: "index.php?controlador=Reorganization&accion=verifysession",
                        dataType: "json",
                        async: false,
                    ...
                    if(mensajeres){
                        window.location="index.php?controlador=Reorganization&accion=verifyReorganization&tipo=rp";
                    }else{
                ...
                parent.message("Verifique la cantidad ingresada para el producto a reorganizar","images/iconos_alerta/error.png");
                $('#loader').css('display', 'none');
                $("#siguiente").removeAttr("disabled");
                $("#siguiente").addClass("buscarButton");
                $("#siguiente").removeClass("buscarButtonDis");
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.res=='si'){
                        ...
                        $.fancybox.close();
                        parent.message("Se ha eliminado el producto a reorganizar","images/iconos_alerta/ok.png");
                    }else{
                        $.fancybox.close();
                        parent.message("No se pudo eliminar el producto a reorganizar","images/iconos_alerta/error.png");
                    }
                }
            });
        });
    ...
    function seleccionar(id,item){
        ...
        if(cant == ""){
            ...
            if($.trim($("#agregarpr"+item).html())=='Agregar'){
                $.ajax({
                    type: "POST",
                    url: "index.php?controlador=Reorganization&accion=addReorganization",
                    dataType: "json",
                ...
                $.ajax({
                    type: "POST",
                    url: "index.php?controlador=Reorganization&accion=deleteItem",
                    dataType: "json",
    ...
    var cant = $.trim($("#cantdev"+item).val());
        if(cant<="0"||cant == ""){
            ...
            parent.message("No puede incluir en la reorganizaci&oacute;n el mismo producto seleccionado en el paso 1","images/iconos_alerta/error.png");
            $("#cantdev"+item).val("");
        }else{
            $("#agregarpr"+item).removeAttr('disabled');
            $("#agregarpr"+item).addClass("buscarButton");
    ...
    function cancelarsesion(id){
        ...
        $('#accept').click(function(){
            $.ajax({
                ...
                success: function(data) {
                    if(data.res=='si'){
                        parent.message("Se ha cancelado la reorganizaci&oacute;n","images/iconos_alerta/ok.png");
                        parent.$.fancybox.close();
                    }else{
                        parent.message("No se pudo cancelar la reorganizaci&oacute;n","images/iconos_alerta/error.png");
                        $.fancybox.close();
                    }
                }
            });
    ...
    function createdata(id,referencia,nombre,stock,idcode,idverify){
        var inputcantid;
        inputcantid="<input name='cantidad' \n\
                            label='cantidad'\n\
                            patt='val2' \n\
                            minsize='1' \n\
                            item='"+id+"' \n\
        ...
        var addId = $('#example2').dataTable().fnAddData([
            ...
            "<a id='dell"+idcode+"' \n\
             ...
             verify='"+idverify+"' \n\
             href='#' \n\
             onclick='confirmfunction($(this).attr(\"id\"))'> <img class='delete' src='images/delete.gif' title='Eliminar item'/> </a>"
        ]);
    ...
    $(document).ready(function(){
        ...
        $("#addToOut").click(function(){
            ...
            if($("#codeprod").val()){
                $.ajax({
                    type: "POST",
                    url: "index.php?controlador=Reorganization&accion=addNewItem",
                    dataType: "json",
        ...
        oTable=$('#example2').dataTable( {
            "fnCreatedRow": function( nRow, aData, iDataIndex ) {
                ...
                if($('td:eq(3) input[name=\'cantidad\']', nRow).attr('objectid')){                                            
                    $('td:eq(2)', nRow).attr('id','stoimportante'+$('td:eq(3) input[name=\'cantidad\']', nRow).attr('objectid'));
                    $('td:eq(3) input[name=\'cantidad\']', nRow).removeAttr('objectid')                    
                }
            },
            "oLanguage":  {
                "sEmptyTable":     "No existen datos disponibles",
